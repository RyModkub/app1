...
export default function FormSelectFile(){
    ...
    const onClickButton = () =>{
        ...
        for(let f of inputFile.current.files){
            if(f.size > maxSize*1000){
                alert(`ขนาดไฟล์แต่ละไฟล์ต้องไม่เกิน ${maxSize} KB`)
                return
            }
        }
        alert('Files OK')
    ...
    return(
        ...
        <form>
            ...
            <div className="form-group mb-3">
                <label htmlFor="file" className="from-label">เลือกไฟล์</label>
                <input type="file" id="file" accept="image/*"
                className="form-select form-control-sm" ref={inputFile} multiple/>
            </div>
